...
class EditViewController : UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func noClicked(_ sender: UIBarButtonItem) {
        ...
        UIView.animate(withDuration: 0.3, animations: {
            ...
            _ = self.textField(self.issuer, shouldChangeCharactersIn: NSRange(), replacementString: "")
        })
    }
    ...
    override func viewWillDisappear(_ animated: Bool) {
        if let t = token {
            _ = Token.store.save(t)
        }
    }

    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if sender as! UIButton == image {
            (segue.destination as! ImageViewController).token = token
        } else {
            _ = TokenStore().erase(token: token)
            token = nil
        }
    }
    ...
    fileprivate func token2UI() {
        ...
        _ = textField(issuer, shouldChangeCharactersIn: NSRange(), replacementString: "")
    }
