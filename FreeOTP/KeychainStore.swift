...
open class KeychainStore<T: KeychainStorable> {
    ...
    open func add(_ storable: T, locked: Bool = false) -> Bool {
        return add(
            storable.account,
    ...
    open func save(_ storable: T) -> Bool {
    ...
    open func erase(_ storable: T) -> Bool {
        return erase(storable.account)
    }

    open func erase(_ account: String) -> Bool {
